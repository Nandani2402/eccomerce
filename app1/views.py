...
def buynow(request):
    if'email' in request.session:
        a=register.objects.get(emil=request.session['email'])
        if request.method=='POST':
            ...
            request.session['useraddress']=a.address   
            b=Product.objects.get(id=request.POST['id'])      
            request.session['orderAmount']=b.price
            request.session['paymentMethod']='Razorpay'
            request.session['transacetionId']=''
            return redirect('razorpayView')
        else:
            return redirect('login')
        
RAZOR_KEY_ID ='rzp_test_iJ2UXPXnrQgcgi'
RAZOR_KEY_SECRET = 'wxnooaWkaL6ZsC4hTh6aOVf8'
